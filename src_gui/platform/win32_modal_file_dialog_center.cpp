...
void CenterHWNDOnMonitorOf(HWND wnd, HWND ref_for_work_area) {
  ...
  const RECT work =
      up::gui::platform::win32::WorkAreaForMonitorContainingWindow(ref_for_work_area ? ref_for_work_area : wnd);
...
LRESULT CALLBACK ModalFileDialogCenterCbtProc(int nCode, WPARAM wParam, LPARAM lParam) {
  ...
  const RECT work_ref = up::gui::platform::win32::WorkAreaForMonitorContainingWindow(g_modal_file_dlg_owner);
