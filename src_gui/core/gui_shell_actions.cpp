...
std::string build_configure_args_line(const std::filesystem::path& up_exe, const std::string& cwd_portable,
  ...
  std::string leaf;
  std::string qerr;
  if (!persist::query_print_build_dir_name(up_exe, std::filesystem::path(cwd_portable), build_dir_field, env, leaf,
                                             qerr)) {
...
std::optional<std::filesystem::path> try_acquire_run_context(const std::filesystem::path& up_exe,
  ...
  if (!std::filesystem::exists(up_exe)) {
...
void run_up_command_in_detached_thread(const std::filesystem::path& up_exe, const std::filesystem::path& cwd_p,
  ...
  std::thread([up_exe, cwd_p, args_no_exe, extra, &busy, log_line, on_done_on_ui]() {
    std::string cmd = persist::shell_single_quote(up_exe.generic_string()) + " " + args_no_exe;
    if (!extra.empty())
      cmd += " " + extra;
    std::string out;
    int code = -1;
