...
**GroundZero**（命令行缩写 **`gz`**）是一个面向「用数据结构驱动构建与包关系」的原型命令行工具：用 **`package.xml`** 与各目标子目录中的 **`target.xml`**（每目录至多一个）描述包与目标，`gz` 负责扫描、生成 CMake 工程、构建、测试与运行。**推荐**用户**纯手写**上述 XML（见 **`doc/zh/getting-started.md`**、**`doc/zh/package-target-xml-spec.md`** 文首）。**`gz reverse` 子命令已移除**；迁移请手写 **`package.xml` / `target.xml`**。**推荐阅读顺序**（各篇职责不重复；**`doc/zh/` 与 `doc/en/` 同名文件均为该语言的完整正文**）：**①** **[doc/zh/getting-started.md](doc/zh/getting-started.md)** / **[doc/en/getting-started.md](doc/en/getting-started.md)** 分步跟做 → **②** **[doc/zh/cli-reference.md](doc/zh/cli-reference.md)** / **[doc/en/cli-reference.md](doc/en/cli-reference.md)** 子命令与全部参数 → **③** **[doc/zh/user-manual.md](doc/zh/user-manual.md)** / **[doc/en/user-manual.md](doc/en/user-manual.md)** 文档分工、易错 FAQ、**gz-gui**。`package.xml` / `target.xml` **字段级**约定见 **[doc/zh/package-target-xml-spec.md](doc/zh/package-target-xml-spec.md)** / **[doc/en/package-target-xml-spec.md](doc/en/package-target-xml-spec.md)** 与 **`gz spec`**；**内置变量、`gz_cache.txt` 与 `GZ_*` 键**见 **[doc/zh/internal-variables.md](doc/zh/internal-variables.md)** / **[doc/en/internal-variables.md](doc/en/internal-variables.md)**；**从最小包到 `when`/预处理/子工程**见 **[doc/zh/script-tutorial.md](doc/zh/script-tutorial.md)** / **[doc/en/script-tutorial.md](doc/en/script-tutorial.md)**；**脚本消息 `trigger` 总表**见 **[doc/zh/script-messages.md](doc/zh/script-messages.md)** / **[doc/en/script-messages.md](doc/en/script-messages.md)**；`doc/` 索引见 **[doc/README.md](doc/README.md)**。设计背景与完整约定见 **[DESIGN.md](DESIGN.md)**，思维导图见 **[mindmap.mmd](mindmap.mmd)**。近期变更记录见 **[CHANGELOG.md](CHANGELOG.md)**。
