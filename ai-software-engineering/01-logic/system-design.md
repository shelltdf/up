...
**`gz_reverse_cmake/`**（可执行名 `gz_reverse_cmake`）与主工程同根 **`add_subdirectory`** 构建，**`install(..., COMPONENT gz_runtime)`** 与 **`gz`**、**`gz-gui`** 同发至 **`bin/`**；逻辑上仍**不纳入**上图（不链接 `gz-lib`，不进入 `gz configure` 运行期），**仅**静态读 **`CMakeLists.txt`** 子集并**生成** `package.xml` / `target.xml` 初稿（**不**在工具内子进程运行 `cmake`；可选 **`--file-api`** 仅**读**用户预置的 File API 回复 JSON 作对照，**不**自动发 File API 查询）。详见 [detailed-design-gz-reverse-cmake.md](detailed-design-gz-reverse-cmake.md) 与 `02-physical/gz-reverse-cmake/`。
