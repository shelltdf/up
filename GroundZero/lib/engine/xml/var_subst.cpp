...
std::map<std::string, std::string> make_builtin_var_map(const std::string& package_name, const std::string& package_version,
  ...
  m["GZ_OS"] = builtin_host_os();
  m["GZ_PACKAGE_NAME"] = package_name;
  m["GZ_PACKAGE_VERSION"] = package_version;
  m["GZ_TARGET_NAME"] = target_name;
  m["GZ_TARGET_BUILD_SYSTEM"] = lower_ascii(build_system);
  m["GZ_CONFIG"] = lower_ascii(config_mode);
  return m;
}
