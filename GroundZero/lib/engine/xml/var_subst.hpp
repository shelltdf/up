...
// Host OS token for GZ_OS (windows | linux | darwin).
std::string builtin_host_os();

// Builtins: GZ_OS, GZ_PACKAGE_NAME, GZ_PACKAGE_VERSION, GZ_TARGET_NAME, GZ_TARGET_BUILD_SYSTEM, GZ_CONFIG.
...
// Merge: builtins, then package_vars, then target_vars, then opts (configure `--opt` / gz_cache); last writer wins.
