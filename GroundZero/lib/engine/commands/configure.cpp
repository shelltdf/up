...
// Primary install prefix first, then extra entries from GZ_CMAKE_PREFIX_PATH (semicolon-separated on all platforms).
std::string merge_cmake_prefix_path_value(const std::filesystem::path& primary_install_abs,
  ...
  const auto it = opts.find("GZ_CMAKE_PREFIX_PATH");
...
void write_gz_cache(const std::filesystem::path& cache_path,
                    const std::filesystem::path& cwd,
...
void write_packages_md(const std::filesystem::path& out_path,
  ...
  f << "Generated by `gz configure` at `" << time_buf << "`.\n\n";
  ...
  f << "Each **name** / **value** pair is a **default** for `@NAME@` substitution and `when=` (after built-ins). "
       "At configure, **package** `<vars>` apply, then **target** `<vars>`, then **`--opt` / `gz_cache.txt`** for the "
       "same key (last wins). Allowed override keys include every `GZ_*` option plus any **C-style "
       "identifier** except reserved cache lines (`cwd`, `arch`, `package`, …); see `gz spec`.\n\n";
  bool any_vars = false;
  ...
  if (any_intra_target_dep) {
    ...
    if (!link_edges.empty()) {
      ...
      f << "No same-package `<dependency name=\"...\"/>` entries were found in `target.xml`. Output is based on "
            "**resolved executable → library** link lines from `gz configure`.\n\n"
...
std::map<std::string, std::string> load_cached_gz_options(const std::filesystem::path& cache_path) {
...
std::map<std::string, std::string> merge_gz_options(const std::map<std::string, std::string>& base,
...
int run_configure(const ConfigureRequest& req) {
  ...
  std::map<std::string, std::string> seed_opts = merge_gz_options({}, opt_kvs);
  std::string seed_build_system = lower_ascii(option_or_compat(
      seed_opts, "GZ_TARGET_BUILD_SYSTEM", "GZ_BUILD_SYSTEM", "cmake"));
  if (!equals_ci(seed_build_system, "cmake") && !equals_ci(seed_build_system, "ninja")) {
    std::cerr << "configure: unsupported GZ_TARGET_BUILD_SYSTEM=" << seed_build_system << " (expected cmake/ninja)\n";
    return 7;
  }
  ...
  auto opts = merge_gz_options(load_cached_gz_options(build_root / "gz_cache.txt"), opt_kvs);
  ensure_default_build_parallel_options(opts);
  const std::string cpu = gz::arch_from_target_cpu(option_or_compat(opts, "GZ_TARGET_CPU_ARCH", "GZ_CPU_ARCH", host_arch));
  ...
  const std::string generator = lower_ascii(option_or_compat(opts, "GZ_CMAKE_GENERATOR", "", ""));
  ...
  const std::string build_system = lower_ascii(option_or_compat(
      opts, "GZ_TARGET_BUILD_SYSTEM", "GZ_BUILD_SYSTEM", seed_build_system));
  ...
  const auto cache_path = build_root / "gz_cache.txt";
  ...
  auto apply_script_command = [&](const LoadedTarget& lt, const std::string& trigger, const std::string& cmd) {
    ScriptExecutionContext ctx;
    ctx.dom_root = &dom;
    ctx.current_node = find_target_node(lt);
    ctx.message_type = script_message_type_from_string(trigger);
    ctx.vars_view = merged_vars_for_target(lt);
    return resolve_script_command(ctx, cmd);
  };
  ...
  if (!equals_ci(build_system, "cmake") && !equals_ci(build_system, "ninja")) {
    std::cerr << "configure: unsupported GZ_TARGET_BUILD_SYSTEM=" << build_system << " (expected cmake/ninja)\n";
    return 7;
  }
  ...
  for (const auto& lt : build_targets) {
    ...
    for (const auto& ae : lt.desc.assets) {
      if (ae.kind == "dir") {
        ...
        asset_dirs.insert({rel_to_cwd(build_root, d), asset_install_dest(ae.to),
  ...
  // (same order as `build_targets`; later entries win on duplicate keys) + workspace. Builtin `GZ_TARGET_NAME` stays
      ...
      for (const auto& s : lt.desc.source_entries) {
        ...
        if (s.kind == "glob") {
          ...
          for (const auto& sf : files) {
            ...
            tm.source_rules.push_back({std::filesystem::absolute(sf).generic_string(),
                                       apply_script_command(lt, "sources.preprocess", s.preprocess_command),
  ...
  {
    const std::string cmake_generator = option_or_compat(opts, "GZ_CMAKE_GENERATOR", "", "");
  ...
  if (!graph_model.cmake_prefix_path.empty())
    cache_opts["GZ_CMAKE_PREFIX_PATH"] = graph_model.cmake_prefix_path;
  ...
  const std::string cmake_generator = option_or_compat(opts, "GZ_CMAKE_GENERATOR", "", "");
  const std::string dbg_cfg = lower_ascii(option_or_compat(opts, "GZ_TARGET_DEBUG", "GZ_DEBUG", "OFF"));
