// Windows Win32：gz-gui 主窗与消息循环（与 Linux platform/gtk/gtk_gui.cpp、macOS platform/cocoa/cocoa_gui.mm 同一职责层级）。
// 仅通过命令行调用与 gz-gui.exe 同目录的 gz.exe；不链接、不包含 GroundZero/ 下业务代码（见 DESIGN.md / mindmap）。
...
#include "platform/win32/win32_gui_ids.hpp"
#include <windowsx.h>
#include <commctrl.h>
#include <commdlg.h>
#include <shellapi.h>
...
#include "platform/win32/win32_encoding.hpp"
#include "platform/win32/win32_fsutil.hpp"
#include "platform/win32/win32_modal_file_dialog_center.hpp"
#include "platform/win32/win32_paths.hpp"
#include "platform/win32/win32_run_process.hpp"
#include "platform/win32/win32_text_util.hpp"
#include "platform/win32/win32_window_placement.hpp"

namespace {

using namespace gz::gui::platform::win32::ids;

using gz::gui::platform::win32::CenterOuterWindowOnScreen;
using gz::gui::platform::win32::CmdExePath;
using gz::gui::platform::win32::DirOfModule;
using gz::gui::platform::win32::GetEnvVarW;
using gz::gui::platform::win32::GuiSettingsPath;
using gz::gui::platform::win32::ExtractXmlAttr;
using gz::gui::platform::win32::NormalizePath;
using gz::gui::platform::win32::NowTimeStamp;
using gz::gui::platform::win32::PathToPortableSlashes;
using gz::gui::platform::win32::ReadTextFileUtf8BestEffort;
using gz::gui::platform::win32::SplitPathList;
using gz::gui::platform::win32::ToLowerAscii;
using gz::gui::platform::win32::Utf8ToWide;
using gz::gui::platform::win32::WideToUtf8;
using gz::gui::platform::win32::DispatchCompleteLines;
using gz::gui::platform::win32::DispatchTailChunk;
...
static void attach_default_gui_icons(WNDCLASSW& wc) {
  ...
  if (!s_loaded) {
    ...
    s_lg = static_cast<HICON>(LoadImageW(hi, MAKEINTRESOURCEW(kGzGuiAppIconResourceId), IMAGE_ICON,
...
// When non-null, TerminateJobObject kills the whole tree (gz -> cmake, etc.) so stdout pipe write ends close.
...
// 与 GroundZero/lib/engine/commands/commands_common.cpp 中 default_parallel_jobs_hardware() 的 Windows 分支保持一致：
...
std::vector<OptionRow> MakeDefaultGuiOptions() {
  ...
  return {
      {L"GZ_TARGET_SYSTEM", L"windows", L"windows | linux | macos | android | ios | emsdk | uwp"},
      {L"GZ_TARGET_CPU_ARCH", L"x86_64", L"x86 | x86_64 | arm | arm64"},
      {L"GZ_TARGET_DYNAMIC_LIBRARY", L"OFF", L"ON | OFF"},
      {L"GZ_TARGET_CRT", L"dynamic_md", L"static_mt | dynamic_md"},
      {L"GZ_TARGET_DEBUG", L"OFF", L"ON | OFF"},
      {L"GZ_TARGET_BUILD_SYSTEM", L"cmake", L"cmake | ninja"},
      {L"GZ_CMAKE_GENERATOR", L"Visual Studio 17 2022", L"(when GZ_TARGET_BUILD_SYSTEM=cmake) VS/Ninja/Makefiles"},
      {L"GZ_BUILD_PARALLEL", par, L"1..512; cmake --build --parallel / ninja -j"},
      {L"GZ_BUILD_JOBS", par, L"alias of GZ_BUILD_PARALLEL (ninja -j)"},
  };
}
...
void SetStatusRunHelpRunning() {
  SetStatus(T(L"正在运行 gz --help…", L"Running gz --help…"));
}

void SetStatusRunHelpFailed() {
  SetStatus(T(L"运行 gz --help 失败", L"Failed to run gz --help"));
}

void SetStatusRunHelpLoaded() {
  SetStatus(T(L"已加载 gz --help", L"gz --help loaded"));
}
...
void LogErrorGzExeNotFound() {
  AppendErrorLog(L"找不到 gz.exe，请与 gz-gui.exe 放在同一目录。", L"gz.exe not found (place next to gz-gui.exe).");
}
...
void LogWarningGeneratorIgnoredForNonCMakeBuildSystem() {
  AppendWarningLog(L"当前 GZ_TARGET_BUILD_SYSTEM 不是 cmake，GZ_CMAKE_GENERATOR 可能不会生效。",
                   L"GZ_TARGET_BUILD_SYSTEM is not cmake; GZ_CMAKE_GENERATOR may be ignored.");
}
...
// 单次 gz 运行的「Run # / vcvars / 命令行」必须与当次 stdout 成组显示：
// - 不走 AppendLog 的去重（否则连续多次相同 [vcvars] 行会在 1200ms 内被静默丢弃）；
// - 用 SendMessage 同步写入，避免与 worker 的 WM_APPEND_RUN_LOG 在队列里交错导致顺序错乱。
void AppendRunBookkeepingToLog(const gz::gui::core::GzLaunchPlan& plan, const std::wstring& vcvars,
                               unsigned long run_no) {
  if (!IsWindow(g_hwnd))
    return;
  const auto now = std::chrono::steady_clock::now();
...
void AppendConfigureEnvArgs(std::wstring& args_no_exe) {
  ...
  append_opt(L"GZ_TARGET_BUILD_SYSTEM", g_env_settings.selected_build_system);
  append_opt(L"GZ_TARGET_COMPILER", g_env_settings.selected_compiler);
  append_opt(L"GZ_ANDROID_SDK", g_env_settings.android_sdk_path);
  append_opt(L"GZ_ANDROID_NDK", g_env_settings.android_ndk_path);
  append_opt(L"GZ_EMSDK_PATH", g_env_settings.emsdk_path);
}
...
bool QueryConfigureBuildDirNameFromGzExe(const std::wstring& gz_exe, const std::wstring& cwd, std::wstring& out_arch_leaf,
                                         std::wstring& err_msg);
...
bool ShowEnvSettingsDialog(HWND owner) {
  ...
  static constexpr wchar_t kEnvClass[] = L"GzGuiEnvSettingsClass";
  if (!cls_registered) {
    WNDCLASSW wc{};
...
std::wstring GzExePath() {
  return (std::filesystem::path(DirOfModule()) / L"gz.exe").lexically_normal().generic_wstring();
}
...
bool PickFolder(HWND owner, const std::wstring& initial_dir, std::wstring& out, const GUID& client_guid) {
  ...
  gz::gui::platform::win32::ModalFileDialogCenterBegin(owner);
  const HRESULT hr_show = dlg->Show(owner);
  gz::gui::platform::win32::ModalFileDialogCenterEnd();
  if (FAILED(hr_show)) {
    dlg->Release();
    return false;
  }
...
std::filesystem::path ResolveGzCachePath(const std::wstring& cwd) {
  if (cwd.empty())
    return {};
  // configure 写入：<cwd>/.intermediate/build/<leaf>/gz_cache.txt，其中 <leaf> 为 --build-dir-name（与 print-build-dir-name 一致），
  // 不一定是 "default"。旧工程可能仍只有 default。
  const auto try_leaf = [&](const std::wstring& leaf) -> std::filesystem::path {
    if (leaf.empty())
      return {};
    std::error_code ec{};
    const auto p = std::filesystem::path(cwd) / L".intermediate" / L"build" / leaf / L"gz_cache.txt";
    if (std::filesystem::exists(p, ec))
      return p;
    return {};
  };

  const std::wstring gz_exe = GzExePath();
  if (GetFileAttributesW(gz_exe.c_str()) != INVALID_FILE_ATTRIBUTES) {
    std::wstring leaf;
    std::wstring err;
    if (QueryConfigureBuildDirNameFromGzExe(gz_exe, cwd, leaf, err) && !leaf.empty()) {
...
std::string ReadGzCacheArchUtf8(const std::filesystem::path& cache_path) {
...
void RefreshInstallDirDisplay(const std::filesystem::path& cache_path, const std::wstring& cwd_w) {
  ...
  const std::string arch_utf8 = ReadGzCacheArchUtf8(cache_path);
  if (arch_utf8.empty()) {
    SetWindowTextW(g_install_dir, L"");
    return;
  }
...
// 与 GroundZero/lib/engine/commands/commands_common.cpp 中 gz_mergeable_option_key 语义对齐（gz-gui 不链接引擎）。
...
void LoadOptionsFromCache(const std::wstring& cwd, bool restore_scan_roots = true) {
  const auto cache = ResolveGzCachePath(cwd);
...
void AppendConfigureScanDirs(std::wstring& args_no_exe) {
  ...
  std::wstring cwd;
  GetEditText(g_path, cwd);
  gz::gui::core::append_scan_args(args_no_exe, dirs, cwd);
}
...
// Shells out to gz.exe (same GZ_* as configure) — no compile-time dependency on GroundZero/.
bool QueryConfigureBuildDirNameFromGzExe(const std::wstring& gz_exe, const std::wstring& cwd, std::wstring& out_arch_leaf,
                                         std::wstring& err_msg) {
  out_arch_leaf.clear();
  err_msg.clear();
  ...
  if (!RunProcessCapture(gz_exe, cmdline, cwd, output, code)) {
...
void HandleProcessDone(const DonePack& pack) {
  ...
  if (!pack.user_stopped && pack.exit_code == 0 && g_last_gz_args.rfind(L"configure", 0) == 0) {
    RefreshRunTargetListFromPath();
    std::wstring cwd;
    GetEditText(g_path, cwd);
    // 与 gz_cache.txt 一致：恢复 GZ_* 与 scan_roots（configure 已写入本次实际扫描根）。
...
gz::gui::core::GzLaunchPlan ResolveLaunchPlan(const std::wstring& gz_exe, const std::wstring& args_no_exe,
  ...
  return gz::gui::core::build_launch_plan(gz_exe, args_no_exe, extra, use_vcvars ? vcvars : L"",
                                          use_vcvars ? CmdExePath() : L"");
}

unsigned long BeginRunUiState(const gz::gui::core::GzLaunchPlan& plan, const std::wstring& vcvars,
                              const std::wstring& args_no_exe) {
  g_abort_run_no.store(0);
  g_last_gz_args = args_no_exe;
...
void SetUiRunning(bool running) {
  ...
  if (menu) {
    ...
    EnableMenuItem(menu, IDM_GZ_HELP, running ? gray : ena);
  ...
  SetStatus(running ? T(L"正在运行 gz…", L"Running gz…") : T(L"就绪", L"Ready"));
}
...
bool FinishConfigureArgsForRun(std::wstring& args_no_exe, std::wstring& err_out) {
  ...
  std::wstring qerr;
  if (!QueryConfigureBuildDirNameFromGzExe(gz_exe, cwd, arch_leaf, qerr)) {
    err_out = T(L"[错误] 无法从 gz.exe 计算 --build-dir-name: ", L"[Error] Cannot resolve --build-dir-name from gz.exe: ");
    err_out += qerr;
    err_out += L"\r\n";
    return false;
  }
...
void RunGzAsync(std::wstring args_no_exe) {
  const std::wstring gz_exe = GzExePath();
  if (GetFileAttributesW(gz_exe.c_str()) == INVALID_FILE_ATTRIBUTES) {
    LogErrorGzExeNotFound();
    return;
  }
  ...
  const auto plan = ResolveLaunchPlan(gz_exe, args_no_exe, extra);
...
void LayoutChildren(HWND hwnd) {
  ...
  // Install Dir（与 gz 的 default_install_root(cwd)/<arch> 一致，arch 来自 gz_cache.txt）
...
bool SoftValidateOptionValue(const OptionRow& opt, const std::wstring& value) {
  ...
  if (opt.name == L"GZ_CMAKE_GENERATOR") {
    const OptionRow* build = FindOptionByName(L"GZ_TARGET_BUILD_SYSTEM");
...
void CreateMainMenu(HWND hwnd) {
  ...
  AppendMenuW(help, MF_STRING, IDM_GZ_HELP, T(L"gz 帮助(&U)…", L"gz &Help…"));
...
void LayoutGzHelpDialog(HWND hwnd, GzHelpDialogState* st) {
  RECT rc{};
  GetClientRect(hwnd, &rc);
  const int pad = 10;
  const int btn_h = 28;
...
LRESULT CALLBACK GzHelpWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  auto* st = reinterpret_cast<GzHelpDialogState*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
  switch (msg) {
    case WM_CREATE: {
      auto* cs = reinterpret_cast<CREATESTRUCTW*>(lParam);
      st = reinterpret_cast<GzHelpDialogState*>(cs->lpCreateParams);
      SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(st));
      const HFONT ui = reinterpret_cast<HFONT>(GetStockObject(DEFAULT_GUI_FONT));
...
void ShowGzHelpDialog(HWND owner, const std::wstring& text) {
  static bool cls_registered = false;
  static constexpr wchar_t kHelpClass[] = L"GzGuiHelpDialogClass";
  if (!cls_registered) {
    WNDCLASSW wc{};
    wc.lpfnWndProc = GzHelpWndProc;
    wc.hInstance = GetModuleHandleW(nullptr);
  ...
  GzHelpDialogState st{};
  st.text = text;
  EnableWindow(owner, FALSE);
  int dlg_x = 0;
  int dlg_y = 0;
  CenterOuterWindowOnScreen(owner, 760, 520, &dlg_x, &dlg_y);
  HWND dlg = CreateWindowExW(WS_EX_DLGMODALFRAME, kHelpClass, T(L"gz 帮助信息", L"gz Help"),
...
void ShowGzHelpInfo(HWND hwnd) {
  const std::wstring gz_exe = GzExePath();
  if (GetFileAttributesW(gz_exe.c_str()) == INVALID_FILE_ATTRIBUTES) {
    MessageBoxW(hwnd, T(L"未找到 gz.exe（需与 gz-gui.exe 同目录）。", L"gz.exe not found (must sit next to gz-gui.exe)."),
                T(L"gz 帮助信息", L"gz Help"), MB_OK | MB_ICONWARNING);
    return;
  }
  SetStatus(T(L"正在运行 gz --help…", L"Running gz --help…"));
  const std::wstring cmd = L"\"" + gz_exe + L"\" --help";
  std::wstring output;
  DWORD exit_code = 0;
  if (!RunProcessCapture(gz_exe, cmd, L"", output, exit_code)) {
    SetStatusRunHelpFailed();
    MessageBoxW(hwnd, T(L"执行 gz --help 失败。", L"Failed to run gz --help."), T(L"gz 帮助信息", L"gz Help"),
                MB_OK | MB_ICONERROR);
    return;
  }
  std::wstringstream ss;
  ...
  ShowGzHelpDialog(hwnd, ss.str());
  SetStatusRunHelpLoaded();
}

struct GzListDialogState {
  std::wstring text;
  std::wstring gz_exe;
  std::wstring cwd;
...
std::wstring BuildGzListDialogTitle(const GzListDialogState* st) {
...
void RefreshGzListDialogTitle(HWND hwnd, const GzListDialogState* st) {
  SetWindowTextW(hwnd, BuildGzListDialogTitle(st).c_str());
  if (st->lbl_hint) {
...
void EnsureGzListTreeImages(GzListDialogState* st) {
...
void FillGzListTree(HWND tree, const std::wstring& text) {
...
void RefreshGzListDetailPanel(GzListDialogState* st) {
...
void LayoutGzListDialog(HWND hwnd, GzListDialogState* st) {
  RECT rc{};
  GetClientRect(hwnd, &rc);
  const int pad = 10;
  const int hint_h = 20;
...
bool ExportDomFromGzList(HWND owner, GzListDialogState* st, const wchar_t* option_flag, const wchar_t* default_name,
  ...
  if (!PickSaveListExportPath(owner, default_name, filter, def_ext, path))
    return false;
  std::wstring cmd = L"\"" + st->gz_exe + L"\" list ";
  cmd += option_flag;
  cmd += L" ";
  cmd += QuoteWinArg(path);
  std::wstring output;
  DWORD exit_code = 0;
  if (!RunProcessCapture(st->gz_exe, cmd, st->cwd, output, exit_code)) {
...
LRESULT CALLBACK GzListWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  auto* st = reinterpret_cast<GzListDialogState*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
  switch (msg) {
    case WM_CREATE: {
      auto* cs = reinterpret_cast<CREATESTRUCTW*>(lParam);
      st = reinterpret_cast<GzListDialogState*>(cs->lpCreateParams);
      SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(st));
      const HFONT ui = reinterpret_cast<HFONT>(GetStockObject(DEFAULT_GUI_FONT));
      ...
      FillGzListTree(st->tree, st->text);
      {
        HTREEITEM root = TreeView_GetRoot(st->tree);
        if (root)
          TreeView_SelectItem(st->tree, root);
      }
      RefreshGzListDetailPanel(st);
      RefreshGzListDialogTitle(hwnd, st);
      LayoutGzListDialog(hwnd, st);
      return 0;
    }
    case WM_SIZE:
      if (st)
    ...
    case WM_COMMAND:
      ...
      if ((HWND)lParam == st->btn_export_json) {
        ...
        if (ExportDomFromGzList(hwnd, st, L"--json", L"dom.json", L"JSON files (*.json)\0*.json\0All files (*.*)\0*.*\0", L"json",
                                &exported)) {
      ...
      if ((HWND)lParam == st->btn_export_xml) {
        std::wstring exported;
        if (ExportDomFromGzList(hwnd, st, L"--xml", L"dom.xml", L"XML files (*.xml)\0*.xml\0All files (*.*)\0*.*\0", L"xml",
                                &exported)) {
          st->last_export_path = exported;
          EnableWindow(st->btn_open_dir, TRUE);
          RefreshGzListDialogTitle(hwnd, st);
...
void ShowGzListDialog(HWND owner, const std::wstring& text, const std::wstring& gz_exe, const std::wstring& cwd) {
  static bool cls_registered = false;
  static constexpr wchar_t kListClass[] = L"GzGuiListDialogClass";
  if (!cls_registered) {
    WNDCLASSW wc{};
    wc.lpfnWndProc = GzListWndProc;
    wc.hInstance = GetModuleHandleW(nullptr);
...
void ShowGzListInfo(HWND hwnd) {
  ...
  SetStatus(T(L"正在运行 gz list…", L"Running gz list…"));
  std::wstring scan_args;
  AppendConfigureScanDirs(scan_args);
  const std::wstring cmd = L"\"" + gz_exe + L"\" list" + scan_args;
  std::wstring output;
  DWORD exit_code = 0;
  if (!RunProcessCapture(gz_exe, cmd, cwd, output, exit_code)) {
    AppendErrorLog(L"执行 gz list 失败。", L"Failed to run gz list.");
    AppendLogRaw(L"$ " + cmd + L"\r\n");
    MessageBoxW(hwnd, T(L"执行 gz list 失败。", L"Failed to run gz list."), T(L"DOM 列表", L"DOM List"), MB_OK | MB_ICONERROR);
    SetStatus(T(L"运行 gz list 失败", L"Failed to run gz list"));
    return;
  }
  if (exit_code != 0) {
    std::wstringstream ss;
    ss << T(L"gz list 执行失败，退出码：", L"gz list failed with exit code: ") << static_cast<unsigned long>(exit_code) << L"\r\n\r\n"
       << output;
    AppendErrorLog(L"gz list 失败（退出码非 0）。", L"gz list failed (non-zero exit code).");
    AppendLogRaw(L"$ " + cmd + L"\r\n" + output + L"\r\n");
    MessageBoxW(hwnd, ss.str().c_str(), T(L"DOM 列表", L"DOM List"), MB_OK | MB_ICONERROR);
    SetStatus(T(L"运行 gz list 失败", L"Failed to run gz list"));
    return;
  }
  ShowGzListDialog(hwnd, output, gz_exe, cwd);
  SetStatus(T(L"已加载 DOM 列表", L"DOM list loaded"));
}

bool IsConfigureOptionName(const std::wstring& name) {
  if (name.rfind(L"GZ_TARGET_", 0) == 0)
    return true;
  if (_wcsicmp(name.c_str(), L"GZ_CMAKE_GENERATOR") == 0)
    return true;
...
bool ShowConfigureOptionDialog(HWND owner) {
  static bool cls_registered = false;
  static constexpr wchar_t kClass[] = L"GzGuiConfigureOptionDialogClass";
  if (!cls_registered) {
    WNDCLASSW wc{};
...
std::wstring InitialWelcomeLogText() {
  if (g_ui_lang_zh) {
    return L"gz-gui：在「当前工作目录 (CWD)」下调用同目录的 gz.exe。\r\n"
           L"「运行参数」可加 --scan 指定额外扫描根（与 CWD 相同的路径不会重复传入）；gz 默认已扫描当前工作目录。\r\n"
  ...
  return L"gz-gui runs gz.exe from the same folder as gz-gui.exe, using the working directory (CWD).\r\n"
         L"Use \"Extra args\" to pass --scan for extra scan roots (duplicates of CWD are skipped); gz already scans CWD.\r\n"
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  switch (msg) {
    ...
    case WM_COMMAND: {
      ...
      if (id == IDM_ABOUT) {
        MessageBoxW(hwnd,
                    ...
                    T(L"关于 gz-gui", L"About gz-gui"), MB_ICONINFORMATION | MB_OK);
        return 0;
      }
      ...
      if (id == IDC_BROWSE) {
        ...
        if (PickFolder(hwnd, init, folder, kPickFolderClientGuidCwd)) {
          ...
          SendMessageW(g_run_target, CB_RESETCONTENT, 0, 0);
          SendMessageW(g_test_target, CB_RESETCONTENT, 0, 0);
          // 若该 CWD 下已有 gz_cache.txt，则一并恢复 scan_roots 到「扫描目录」列表。
    ...
    case WM_CLOSE:
      if (g_running) {
        if (MessageBoxW(hwnd, T(L"gz 仍在运行，确定要关闭窗口吗？", L"gz is still running. Close the window?"),
                        L"gz-gui", MB_YESNO | MB_ICONQUESTION) != IDYES)
          return 0;
      }
      DestroyWindow(hwnd);
      return 0;
