...
void build_window() {
  ...
  cs().gz_exe = persist::executable_parent_dir() / "gz";
  (void)persist::load_settings(cs().settings_file, cs().persist);
  cs().scan_rows = [NSMutableArray array];

  {
    const std::filesystem::path icon_path = persist::executable_parent_dir() / "gz_gui.png";
  ...
  [win setTitle:@"gz-gui"];
  GzGuiCtrl* ctrl = [[GzGuiCtrl alloc] init];
  ...
  NSMenuItem* appItem = [[NSMenuItem alloc] initWithTitle:@"gz-gui" action:nil keyEquivalent:@""];
  NSMenu* appMenu = [[NSMenu alloc] initWithTitle:@"gz-gui"];
  NSMenuItem* quitItem = [[NSMenuItem alloc] initWithTitle:@"Quit gz-gui" action:@selector(terminate:) keyEquivalent:@"q"];
  ...
  log_line("gz-gui (Cocoa) — settings: " + cs().settings_file.generic_string());
}
