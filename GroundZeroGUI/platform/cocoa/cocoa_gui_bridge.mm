...
void run_up_async(const std::string& args_no_exe) {
  ...
  const auto maybe_cwd = shell::try_acquire_run_context(cs().gz_exe, cwd, cs().busy, [](const std::string& s) { log_line(s); });
  if (!maybe_cwd)
    return;
  const std::string extra = ns_to_utf8([cs().tf_extra stringValue]);
  shell::run_gz_command_in_detached_thread(
      cs().gz_exe, *maybe_cwd, args_no_exe, extra, cs().busy, [](const std::string& s) { log_line(s); },
      []() { set_status("Ready"); });
