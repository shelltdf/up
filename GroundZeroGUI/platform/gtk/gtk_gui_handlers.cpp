...
void run_up_line_async(const std::string& args_no_exe) {
  ...
  const auto maybe_cwd = shell::try_acquire_run_context(gs().gz_exe, cwd, gs().busy, [](const std::string& s) { log_line(s); });
  if (!maybe_cwd)
    return;
  const std::string extra = get_entry(gs().entry_extra);
  shell::run_gz_command_in_detached_thread(gs().gz_exe, *maybe_cwd, args_no_exe, extra, gs().busy,
  ...
  set_status("Running gz…");
}

void on_configure(GtkWidget*, gpointer) {
  const std::string cwd = persist::path_to_portable_utf8(get_entry(gs().entry_cwd));
  const std::string cfg = shell::build_configure_args_line(gs().gz_exe, cwd, get_entry(gs().entry_build), collect_scans(),
