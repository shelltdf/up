...
// 各平台共用 UTF-8 的 `gz_gui_settings.txt` 与「POSIX 单引号风格」子进程命令行；Win32 上 `run_shell_in_dir` 通过 PowerShell -EncodedCommand 执行等价脚本（主界面仍由 win32_gui 宽字符实现）。
std::filesystem::path settings_path_near_executable();

// 可执行文件所在目录（不含文件名）。
std::filesystem::path executable_parent_dir();

// 与 Win32 相同的「环境 → configure --opt」键（不含选项表里的 GZ_*，由界面其它入口维护）。
struct PersistedEnv {
...
// 调用 `gz print-build-dir-name`；失败返回 false 并写 err。
