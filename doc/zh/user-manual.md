...
- **目标级依赖**：写在 `target.xml` 的 `<dependency name="..."/>`
  ...
  - **`when`** 目前**不**作用于 `<dependency/>`；哪些标签支持条件、表达式语法以 **`up spec`** 与 **[package-target-xml-spec.md](package-target-xml-spec.md)** 为准。
...
3. **`up configure` / `up build`**（见 §4.3）；在消费方 **`target.xml`** 用 **`<dependency name="pkg:target"/>`** 引用。
