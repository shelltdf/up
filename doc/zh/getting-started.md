...
本教程按**步骤**扩展一个 `up` 工程，与仓库内 **`test_projects/`** 示例一致处会给出引用。字段级语义以 **`package-target-xml-spec.md`** 与 **`up spec`** 为准；命令行以 **`user-manual.md`** 与根目录 **`README.md`**（[`../../README.md`](../../README.md)）为准。**`doc/` 索引导航**：[`../README.md`](../README.md)。英文对照：[`../en/getting-started.md`](../en/getting-started.md)。
...
执行：`up configure` → `up build --build-dir-name default` → `up print-build-dir-name --build-dir-name default` → `up run --install-dir-name <arch 名> hello`。
...
- 若可执行文件**没有任何** `<dependency>`，configure 会按 **`UP_TARGET_DYNAMIC_LIBRARY`** 等策略**自动链接本包内符合条件的库**；若存在**任意**显式 `<dependency>`，则**只**使用你声明的链接关系（见 **`package-target-xml-spec.md` §3.6**）。
...
- 常用内置键：**`UP_OS`**（`windows` / `linux` / `darwin`）、**`UP_CONFIG`**（`debug` / `release`）、**`UP_TARGET_BUILD_SYSTEM`**（`cmake` / `ninja`）等，见 **`internal-variables.md`**。
...
**思路**：Qt 由 **官方安装或包外 CMake install** 提供；`up` **只手写**源、宏、**`<dependency>`** 指向 **`imported_*`** 或本包自编译库；**moc / uic / rcc** 用 **`<preprocess>`** 或包在 **`static_library`** 里以规避 CMake 后端对 **exe 源级 preprocess** 的限制。

| 原 CMake/Qt 概念 | 在 `up` 中的对应 |
|------------------|------------------|
...
| **需打补丁、多步配置** | 仍在**包外**脚本化上游构建；`up` 侧用 **`preprocess`** 或 vendored 源码目标承接生成物，见 **`script-tutorial.md`**。 |
| **与本仓库另一 `up` 包协作** | 多包 **`<dependency name="包名"/>`** + **`包名:目标名`**。 |

---

## 检查清单（发布前）

- [ ] **`up configure`** 无报错；查看 **`.intermediate/build/<叶子>/packages.md`** 中变量与依赖是否符合预期。  
- [ ] **`when`** 仅用于已支持标签；复杂条件用多个键 + **`--opt`** 预置。  
- [ ] 跨包引用已在 **`package.xml`** 声明 **`dependency`**。  
- [ ] **`up run`** 使用正确的 **`--install-dir-name`**（与 **`up_cache.txt` `arch=`** 一致）。  
- [ ] 第三方许可证与预置二进制是否允许分发。  
