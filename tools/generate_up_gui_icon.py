#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate square 1:1 up-gui icon (mushroom cloud / ground-zero) into src_gui/resources/.

Strategy (with ``--ai-master``):
  * **AI raster** for ``up_gui.png`` (512) and ICO **256×256** (sharp, colorful).
  * **LOD vector** for ICO **16 / 32 / 48** — native-drawn for legibility, **colors sampled**
  ...
  python tools/generate_up_gui_icon.py --ai-master path/to.png

Procedural-only (no AI file):

  python tools/generate_up_gui_icon.py
"""
...
def write_lod_outputs(out_dir: Path) -> None:
    ...
    png_path = out_dir / "up_gui.png"
    ico_path = out_dir / "up_gui.ico"
...
def main() -> int:
    ...
    out_dir = root / "src_gui" / "resources"
    out_dir.mkdir(parents=True, exist_ok=True)

    ap = argparse.ArgumentParser(description="Generate up_gui icon assets.")
    ap.add_argument(
        "--ai-master",
        type=Path,
        default=None,
